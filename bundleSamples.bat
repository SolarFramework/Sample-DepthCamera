...
SET filename=SolARSample_DepthCamera_%version%
SET arg1=%1
...
echo "**** Bundle dependencies in bin folder"
 FOR /D /R %%d IN (SolARSample*) DO (
    For %%f IN (%%~fd\*_conf.xml) DO (
      ...
      remaken bundleXpcf "%%f" -d ./bin/x86_64/Release -s modules
      remaken bundleXpcf "%%f" -d ./bin/x86_64/Debug -s modules -c debug
   )
)
...
"7z.exe" a -tzip bin\%filename%_debug.zip bin\x86_64\Debug
"7z.exe" a -tzip bin\%filename%_release.zip bin\x86_64\Release
